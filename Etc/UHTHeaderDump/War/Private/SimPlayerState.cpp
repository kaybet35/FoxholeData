...
ASimPlayerState::ASimPlayerState(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    this->TeamId = -1;
    this->Level = 0;
    this->Rank = 0;
    this->XP = 0;
...
void ASimPlayerState::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
    ...
    DOREPLIFETIME(ASimPlayerState, SquadLeaderDirection);
