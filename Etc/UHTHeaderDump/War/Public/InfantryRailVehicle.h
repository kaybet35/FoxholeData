...
#include "InfantryRailVehicle.generated.h"

UCLASS(Blueprintable)
class WAR_API AInfantryRailVehicle : public ARailVehicle, public IGunnerSupport {
    GENERATED_BODY()
public:
    UPROPERTY(EditAnywhere, ReplicatedUsing=OnRep_GunnerYawAndPitch, meta=(AllowPrivateAccess=true))
    FVector2D GunnerYawAndPitch[3];
    
    UPROPERTY(EditAnywhere, Replicated, SaveGame, meta=(AllowPrivateAccess=true))
    FName StashedAmmoType[3];
    ...
    AInfantryRailVehicle(const FObjectInitializer& ObjectInitializer);
